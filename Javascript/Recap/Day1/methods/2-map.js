...
const newArr = [];
numbers.map(function toInt(arr) {
...
floatNumbers.map(function pennywise(arr){
    newFloat.push(parseFloat(arr));
    return newFloat;
})
...
const movies = [
  ...
  const names = [];
  movies.map(function movieNames (obj){
      names.push(obj.name);
      return names;
  });
...
//4
const movieObjects = [];
movies.map(function movieInfo(obj){
    movieObjects.push({ name: obj.name, genres: obj.genre });
    return movieObjects;
});
